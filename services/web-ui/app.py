...
See also `docs/first_analysis.md` in the repository.
    ...
    if not uploaded:
        st.info("Upload a recording (or use the example CSV) to begin analysis.")
        return

    # Always load overview preview so the user can pick a region
    ...
    # Persist window per file; default = full recording
    default_window = (t_min, t_max)
    if st.session_state.get("analysis_window") is None:
        st.session_state["analysis_window"] = default_window
    prev_win = st.session_state["analysis_window"]
    # Clamp if switching files changed bounds
    win_start = min(max(prev_win[0], t_min), t_max)
    win_end = max(min(prev_win[1], t_max), t_min)
    if win_end <= win_start:
        win_start, win_end = t_min, t_max

    st.subheader("1. Select analysis region")
    st.plotly_chart(
        overview_figure(preview["time"], preview["current"], win_start, win_end),
        use_container_width=True,
    )

    # Streamlit slider step: ~1000 steps across the recording
    span = t_max - t_min
    ...
    selected = st.slider(
        ...
        value=(float(win_start), float(win_end)),
        step=float(step),
    ...
    # Crop preview arrays for threshold tuning
    ...
    with event_tab:
        ...
        if result:
            ...
            if result.get("window_start_s") is not None:
                c4.metric(
                    "Window (s)",
                    f"{result['window_start_s']:.3f}–{result.get('window_end_s', 0):.3f}",
                )
